...
var tooskiTeams = {
	...
	showNews: function(newsId, teamId) {
		...
		news = $.grep(news, function(n, i) {
			return parseInt(tooskiTeams.urldecode(n.id)) == parseInt(newsId);
		});
		news = news[0];
	...
	selectPicture: function(src) {
		if (src == 'camera') {
			navigator.camera.getPicture(tooskiTeams.setAttributePicture, tooskiTeams.cameraError, {
